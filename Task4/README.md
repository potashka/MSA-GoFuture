...
[c4-data-pipeline.puml](c4-data-pipeline.puml) описывают архитектуру конвейера обработки данных и ML-платформы, а [02-data-quality.md](02-data-quality.md) — механизмы обеспечения качества и достоверности данных, проходящих через этот конвейер. Эти материалы дополняют друг друга и предназначены для совместного рассмотрения.
...
закрытая зона сырых данных остаётся в соответствующем регионе, операционная аналитика выполняется на псевдонимизированных данных с контролем доступа, а в центральный аналитический регион по базовой модели реплицируются только агрегированные анонимизированные наборы данных.
